...
# Matches: M08438/Stimulation/000398 inside the longer path
extract_network_path <- function(input_string) {
  # Regex explanation:
  # M\d+        -> Matches "M" followed by digits (e.g., M08438)
  # /           -> Literal slash
  # [A-Za-z]+   -> Matches letters (e.g., Network, Stimulation)
  # /           -> Literal slash
  # \d+         -> Matches digits (e.g., 000398)
...
# NOTE: Ensure 'edge_list' is loaded with your new CSV data before running this!
# Example: edge_list <- read.csv("path_to_MOvsSO_data.csv")

filenames = unique(edge_list$File)
filenames = filenames[-c(2)]
filenames
...
for (file in filenames) {
  ...
  # Fallback if regex fails on unexpected path structure
  if (is.na(path_part)) {
    path_part <- basename(dirname(file))
  }
  ...
  # Grid: 2 rows, 3 columns (Wells 0-5)
  ...
  # Main Title using extracted Mxxxxx path
